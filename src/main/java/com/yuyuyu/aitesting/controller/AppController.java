...
import com.yuyuyu.aitesting.common.*;
...
import com.yuyuyu.aitesting.model.dto.app.AppAddRequest;
import com.yuyuyu.aitesting.model.dto.app.AppEditRequest;
import com.yuyuyu.aitesting.model.dto.app.AppQueryRequest;
import com.yuyuyu.aitesting.model.dto.app.AppUpdateRequest;
...
import com.yuyuyu.aitesting.model.vo.AppVO;
...
public class AppController {
    ...
    @PostMapping("/review")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> adminEditApp(@RequestBody ReviewRequest reviewRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(reviewRequest == null, ErrorCode.PARAMS_ERROR);
        Long id = reviewRequest.getId();
        Integer status = reviewRequest.getReviewStatus();
        //数据校验
        ReviewStatusEnum enumByValue = ReviewStatusEnum.getEnumByValue(status);
        if (id==null||enumByValue == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        //判断应用id是否存在
        App oldApp = appService.getById(id);
        ThrowUtils.throwIf(oldApp == null, ErrorCode.NOT_FOUND_ERROR,"应用id不存在");
        //已是该状态
        if (oldApp.getReviewStatus().equals(status)) {
