...
import com.yuyuyu.aitesting.model.dto.question.*;
...
import com.yuyuyu.aitesting.model.enums.AppTypeEnum;
...
public class QuestionController {
     ...
     * @return
     */
    private String generateQuestionUserMessage(App app,int questionNumber, int optionNumber) {
        ...
        userMassage.append(AppTypeEnum.getEnumByValue(app.getAppType()).getText()+"类").append("\n");
        userMassage.append(questionNumber).append("\n");
        userMassage.append(optionNumber);
    ...
    public BaseResponse<List<QuestionContentDTO>> aiGenerateQuestion(
        ...
        //生成用户题目信息
        String userMessage = generateQuestionUserMessage(app,questionNumber,optionNumber);
        //调用ai生成题目
    ...
    public SseEmitter aiGenerateQuestionSSE(AIGenerateQuestionRequest aiGenerateQuestionRequest,HttpServletRequest request) {
        ...
        //左括号计数器，除了默认值外，当左括号值为0的时候，说明左右括号数量相等，可以截取
        //这里使用原子类，因为流的处理是异步的
        ...
        //调用ai生成题目
        Flowable<ModelData> flowerble = aiManager.doStableStreamRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage);
        flowerble.observeOn(scheduler)
                .map(modledata->modledata.getChoices().get(0).getDelta().getContent())
                .map(message->message.replaceAll("\\s",""))
                .filter(StrUtil::isNotBlank)
                .flatMap(
                        //字符串转为流，接收的是枚举类型，需要用list集合
                        message->{
